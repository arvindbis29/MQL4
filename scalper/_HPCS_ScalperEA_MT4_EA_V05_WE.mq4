//+------------------------------------------------------------------+
//|                                _HPCS_ScalperEA_MT4_EA_V01_WE.mq4 |
...
input double id_lotSize = 0.1;                     // Lots Size
...
//---
   ...
   if(x==0 && y==0)
     {
      ...
      if((iMA(NULL,0,5,0,MODE_EMA,PRICE_CLOSE,0)>iMA(NULL,0,8,0,MODE_EMA,PRICE_CLOSE,0))&&(iMA(NULL,0,8,0,MODE_EMA,PRICE_CLOSE,0)>iMA(NULL,0,13,0,MODE_EMA,PRICE_CLOSE,0))&&(iRSI(NULL,0,14,PRICE_CLOSE,0)<50)&&(iRSI(NULL,0,14,PRICE_CLOSE,1)>iRSI(NULL,0,14,PRICE_CLOSE,0)))
        {
         if((iMA(NULL,0,5,0,MODE_EMA,PRICE_CLOSE,1)<=iMA(NULL,0,8,0,MODE_EMA,PRICE_CLOSE,1))||(iMA(NULL,0,5,0,MODE_EMA,PRICE_CLOSE,1)<=iMA(NULL,0,13,0,MODE_EMA,PRICE_CLOSE,1))||(iMA(NULL,0,8,0,MODE_EMA,PRICE_CLOSE,1)<=iMA(NULL,0,13,0,MODE_EMA,PRICE_CLOSE,1)))
               ...
               if(ii_lotstype ==1)
                 {
                  ...
                  if(ii_lotstype == 2)
                    {
                     ld_lots= CalculateLotUsingRiskPercentageOrRiskAmount(0,risk_p,0,OP_BUY,id_stopLoss);;
                    }
      ...
      if((iMA(NULL,0,13,0,MODE_EMA,PRICE_CLOSE,0)>iMA(NULL,0,8,0,MODE_EMA,PRICE_CLOSE,0))&&(iMA(NULL,0,8,0,MODE_EMA,PRICE_CLOSE,0)>iMA(NULL,0,5,0,MODE_EMA,PRICE_CLOSE,0))&&((iRSI(NULL,0,14,PRICE_CLOSE,0)>50))&&(iRSI(NULL,0,14,PRICE_CLOSE,1)<iRSI(NULL,0,14,PRICE_CLOSE,0)))
        {
         if((iMA(NULL,0,5,0,MODE_EMA,PRICE_CLOSE,1)>=iMA(NULL,0,8,0,MODE_EMA,PRICE_CLOSE,1))||(iMA(NULL,0,5,0,MODE_EMA,PRICE_CLOSE,1)>=iMA(NULL,0,13,0,MODE_EMA,PRICE_CLOSE,1))||(iMA(NULL,0,8,0,MODE_EMA,PRICE_CLOSE,1)>=iMA(NULL,0,13,0,MODE_EMA,PRICE_CLOSE,1)))
               ...
               if(ii_lotstype ==1)
                 {
                  ...
                  if(ii_lotstype ==2)
                    {
                     ld_lots= CalculateLotUsingRiskPercentageOrRiskAmount(0,risk_p,0,OP_SELL,id_stopLoss);
                    }

               int li_ticket = PlaceMarketOrder_Generic(OP_SELL,id_lotSize,_Symbol,id_stopLoss,id_takeProfit,ii_slippage,ii_magicNumber,"FIrst Order");
               if(li_ticket <0)
                 {
                  Print("Order Send Failed ! ", GetLastError());
                 }
   ...
   if((((iMA(NULL,0,5,0,MODE_EMA,PRICE_CLOSE,1)==iMA(NULL,0,8,0,MODE_EMA,PRICE_CLOSE,1))||(iMA(NULL,0,5,0,MODE_EMA,PRICE_CLOSE,1)==iMA(NULL,0,13,0,MODE_EMA,PRICE_CLOSE,1)))&&(iMA(NULL,0,5,0,MODE_EMA,PRICE_CLOSE,0)>iMA(NULL,0,8,0,MODE_EMA,PRICE_CLOSE,0))&&(iMA(NULL,0,8,0,MODE_EMA,PRICE_CLOSE,0)>iMA(NULL,0,13,0,MODE_EMA,PRICE_CLOSE,0))&&((iRSI(NULL,0,14,PRICE_CLOSE,0)<50))&&(iRSI(NULL,0,14,PRICE_CLOSE,1)>iRSI(NULL,0,14,PRICE_CLOSE,0)))||(((iMA(NULL,0,5,0,MODE_EMA,PRICE_CLOSE,1)==iMA(NULL,0,8,0,MODE_EMA,PRICE_CLOSE,1))||(iMA(NULL,0,5,0,MODE_EMA,PRICE_CLOSE,1)==iMA(NULL,0,13,0,MODE_EMA,PRICE_CLOSE,1)))&&(iMA(NULL,0,13,0,MODE_EMA,PRICE_CLOSE,0)>iMA(NULL,0,8,0,MODE_EMA,PRICE_CLOSE,0))&&(iMA(NULL,0,8,0,MODE_EMA,PRICE_CLOSE,0)>iMA(NULL,0,5,0,MODE_EMA,PRICE_CLOSE,0))&&((iRSI(NULL,0,14,PRICE_CLOSE,0)>50))&&(iRSI(NULL,0,14,PRICE_CLOSE,1)<iRSI(NULL,0,14,PRICE_CLOSE,0))))
     {
      y=1;
