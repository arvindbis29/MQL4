//+------------------------------------------------------------------+
//|                                  _HPCS_Adva_MT4_EA_04_V01_WE.mq4 |
...
void OnTick()
  {
//---
   double ld_buy = iCustom(_Symbol, _Period, " _HPCS_WPRTrend_MT4_Indi_V01_WE",0,0);
   double ld_Sell = iCustom(_Symbol, _Period, " _HPCS_WPRTrend_MT4_Indi_V01_WE",1,0);
      ...
      if(gdt_previous != Time[0])
        {
         ...
         li_ticket = PlaceMarketOrder_Generic(OP_BUY,id_lots,_Symbol,0,0,ii_slippage,ii_magicno);
         if(li_ticket < 0)
           {
   ...
   if(ld_Sell != EMPTY_VALUE)
     ...
     SetVirtualTP(ii_takeprofit);
     SetVirtualSL(ii_stoploss);
  }
//+------------------------------------------------------------------+
...
//+------------------------------------------------------------------+
void SetVirtualTP(int VirtualTakeProfit)
{
   ...
   for(int i =0;i < total;i++)
   {
      ...
      {
         if(OrderMagicNumber() == Magic_Number &&OrderSymbol() == Symbol()&& OrderType()<=1)
         {           
            if(OrderType()==OP_BUY)
            {
               if( OrderClosePrice()>= OrderOpenPrice()+ VirtualTakeProfit*Point*_factor)
               {
                    if(OrderClose(OrderTicket(),OrderLots(),OrderClosePrice(),Slippage,clrNONE))
                  {
                     Print("Buy Order hit the  Virtual TP, closed forcefuly");
                  }
                  else
                  {
                     Print("Buy Closed forcefuly falied while  Virtual TP with Error #",GetLastError());
                  }
            ...
            else if(OrderType()==OP_SELL)
            {
               if( OrderClosePrice() <= OrderOpenPrice()-VirtualTakeProfit*Point*_factor)
               {
                  if(OrderClose(OrderTicket(),OrderLots(),OrderClosePrice(),Slippage,clrNONE))
                  {
                     Print("Sell Order hit the  Virtual TP, closed forcefuly");
                  }
                  else
                  {
                     Print("Sell Closed forcefuly falied while  Virtual TP with Error #",GetLastError());
                  }
                  break;
               }
            }
...
void SetVirtualSL(int VirtualStopLoss)
{
   int total = OrdersTotal();
   ...
   for(int i =0;i < total;i++)
   {
      if(OrderSelect(i,SELECT_BY_POS,MODE_TRADES))
      {
         ...
         {           
            if(OrderType()==OP_BUY)
            {
                if( OrderClosePrice()<= OrderOpenPrice()- VirtualStopLoss*Point*_factor)
               {
                   if(OrderClose(OrderTicket(),OrderLots(),OrderClosePrice(),Slippage,clrNONE))
                  {
                     Print("Buy Order hit the  Virtual SL, closed forcefuly");
                  }
                  else
                  {
                     Print("Buy Closed forcefuly falied while  Virtual SL with Error #",GetLastError());
                  }
                  break;
               }
            }
            else if(OrderType()==OP_SELL)
            {
                if( OrderClosePrice()>=OrderOpenPrice()+ VirtualStopLoss*Point*_factor)
               {
                  if(OrderClose(OrderTicket(),OrderLots(),OrderClosePrice(),Slippage,clrNONE))
                  {
                     Print("Sell Order hit the  Virtual SL, closed forcefuly");
                  }
                  else
                  {
                     Print("Sell Closed forcefuly falied while  Virtual SL with Error #",GetLastError());
                  }
                  break;
               }
            }
